...
function renderJsx(name: string, className: string, childrenJSX?: string, styles?: any, props?: any, options?: ComponentGeneratorOptions): string {
  ...
  const classNameString = className ? `className={styles.${className}}` : ''
...
function renderComponentJSX(content: any, styles: any, target: Target, options: ComponentGeneratorOptions): any {
  ...
  if (children && children.length > 0) {
    if (typeof children === 'string') {
      ...
      childrenJSX = children.map((child) => renderComponentJSX(child, styles, target, options))
    }
  }
  ...
  if (mappedProps.children && Array.isArray(mappedProps.children)) {
    childrenJSX = mappedProps.children.map((child) => renderComponentJSX(child, styles, target, options))
  ...
  return renderJsx(mappedType, className, childrenJSX, styles, mappedProps, options)
}

export default class CSSModulesReactComponentRenderer extends ComponentRenderer {
  public render(name: string, content: string, dependencies: any = {}, styles, props, target: Target, options?: ComponentGeneratorOptions): FileSet | null {
    ...
    const jsx = renderComponentJSX(content, styles, target, options)
    ...
    const styleFileContent = jsstocss.stylesheet(styles).css
